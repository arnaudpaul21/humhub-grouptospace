...
namespace humhub\modules\grouptospace\models\forms;
...
class InviteForm extends Model
{
    ...
    public function rules()
    {
        ...
        return array(
            ...
            array('inviteGroups', 'checkInviteGroups'),
    ...
    public function attributeLabels()
    {
        return array(
            ...
            'inviteGroups' => Yii::t('SpaceModule.forms_SpaceInviteForm', 'Groups'),
    ...
    /**
     * Form Validator which checks the invite groups field
     *
     * @param type $attribute
     * @param type $params
     */
    public function checkInviteGroups($attribute, $params)
    {

        // Check if id field is not empty
        if ($this->$attribute != "" && intval($this->$attribute)) {

            $group_id = intval($this->$attribute);

            // Try load group
            $group = Group::findOne(['id' => $group_id]);
            if ($group != null) {
                $this->invitesGroup[] = $group;

            } else {
                $this->addError($attribute, Yii::t('SpaceModule.forms_SpaceInviteForm', "Group not found!"));
            }
        }
    }
    ...
    public function getInviteGroups() {
